...
def attach_fencer_ids(bout_rows, fencer_map):
    for row in bout_rows:
        ...
        row["fencer_a_id"] = fencer_map.get(row.pop("fie_fencer_id_a", None))
        row["fencer_b_id"] = fencer_map.get(row.pop("fie_fencer_id_b", None))
        row["winner_id"] = fencer_map.get(winner_fie_id) if winner_fie_id else None
    return bout_rows
